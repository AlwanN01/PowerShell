...
oh-my-posh init pwsh --config "C:\Users\alwan\Documents\PowerShell\slim.omp.json" | Invoke-Expression
Import-Module -Name Terminal-Icons

Set-PSReadLineKeyHandler -Chord alt+w -Function HistorySearchBackward
Set-PSReadLineKeyHandler -Chord alt+s -Function HistorySearchForward

Set-PSReadLineOption -PredictionViewStyle ListView
Set-PSReadLineOption -AddToHistoryHandler {
    param($command)
    if ($command -like ' *') {return $false}   
    if ($command -like 'node "node_modules/jest/bin/jest.js" *') {return $false}
    if ($command -like 'node .\dist\main.js *') {return $false}
    return $true
} 
